...
def update_supabase_products_to_cars_accurate():
    """Supabaseの商品データを画像に基づいた正確な車データに更新"""
    ...
    try:
        ...
        print("🚗 既存商品を正確な車データに更新中...")
        
        # 外部キー制約があるテーブルを先にクリア
        cursor.execute("DELETE FROM reviews")
        cursor.execute("DELETE FROM cart_items")
        cursor.execute("DELETE FROM order_items")
        print("🗑️  関連データをクリアしました")
        
        # 既存商品を削除
        cursor.execute("DELETE FROM products")
        print("🗑️  既存商品をクリアしました")
        ...
        print(f"\n🎉 Supabaseに16台の正確な車データを追加しました！")
        print("画像ファイル名に基づいた正確なブランド・車種名で更新完了。")
        
    except Exception as e:
        print(f"❌ エラーが発生しました: {e}")

if __name__ == "__main__":
    update_supabase_products_to_cars_accurate()